/*
 * Storyraise Donor Signals page: video chapters and the interactive demo tour.
 *
...
(function () {
    ...
    function openStop(index, focusTitle) {
        ...
        current = index;
        replica.closeDialog();
        // Each stop names the group and list view it wants, so a visitor who has been
        // clicking around lands on the state the copy is describing.
        var want = stop.prepare || {};
        var now = replica.getState();
        if (want.tile && now.tile !== want.tile) replica.setTile(want.tile);
        if (want.view && replica.getState().view !== want.view) replica.setView(want.view);
    ...
    function setMode(next) {
        // Exploring starts from the state the page opens in: New gifts, Overview.
        if (next === 'explore' && mode !== 'explore' && replica && popover.hidden) {
            replica.closeDialog();
            replica.setTile('new_gifts');
            replica.setView('feed');
