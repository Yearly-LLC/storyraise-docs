...
//
//   node video/tools/post-render.mjs [renders/automated-donor-journeys-tour.mp4] [--poster-at 36]
//
...
//    frame, then copies the MP4 to automated-donor-journeys/media/.
...
const input = path.resolve(VIDEO, args.find((a) => !a.startsWith('--') && !/^\d/.test(a)) || 'renders/automated-donor-journeys-tour.mp4');
...
await sharp(frame).jpeg({ quality: 84, mozjpeg: true }).toFile(path.join(MEDIA, 'automated-donor-journeys-tour-poster.jpg'));
await sharp(frame).resize(1200, 630, { fit: 'cover', position: 'centre' }).png({ compressionLevel: 9 }).toFile(path.join(MEDIA, 'og-automated-donor-journeys.png'));
fs.copyFileSync(output, path.join(MEDIA, 'automated-donor-journeys-tour.mp4'));

for (const f of ['automated-donor-journeys-tour.mp4', 'automated-donor-journeys-tour-poster.jpg', 'og-automated-donor-journeys.png', 'automated-donor-journeys-tour.en.vtt', 'chapters.en.vtt']) {
