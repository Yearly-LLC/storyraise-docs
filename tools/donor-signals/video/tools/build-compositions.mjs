...
/*
    The close is three beats now: the line about the data you already have, the setup
    steps, then the card. Each gets its own composition over one narration take, so the
    read stays continuous and only the picture changes under it.
*/
const hosts = {
    s01: { start: S.s01.start, end: cue('s02', 'strip') + 0.1 },
    ui: { start: cue('s02', 'strip') - 0.1, end: end('s09') },
    open: { start: S.s10.start, end: cue('s10', 'steps') - 0.25 },
    setup: { start: cue('s10', 'steps') - 0.35, end: cue('s10', 'cta') - 0.2 },
    s10: { start: cue('s10', 'cta') - 0.45, end: DURATION },
};
...
const setup = {
    start: hosts.setup.start,
    duration: hosts.setup.duration,
    stepAt: [cue('s10', 'steps') + 0.3, cue('s10', 'step2') + 0.1, cue('s10', 'step3') + 0.1].map((t) => round(t, 3)),
    steps: ['Connect your CRM', 'Sync donors and giving', 'Scores ready instantly'],
    logos: ['renxt', 'salesforce', 'bloomerang', 'virtuous', 'donorperfect', 'lgl', 'neon', 'slate', 'ellucian']
        .map((slug) => `assets/logo-${slug}.png`),
};
const openHost = { start: hosts.open.start, duration: hosts.open.duration };
const outroHost = { start: hosts.s10.start, duration: hosts.s10.duration };
fs.writeFileSync(path.join(ASSETS, 'timeline.js'),
    ...
    + `window.SR_SETUP = ${JSON.stringify({ setup, open: openHost, outro: outroHost, steps: setup.steps, logos: setup.logos })};\n`);
...
${['s01', 'ui', 'open', 'setup', 's10'].filter((id) => hosts[id]).map((id, i) => host(id, i + 1)).join('\n')}
