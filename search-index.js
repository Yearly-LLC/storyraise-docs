...
const SEARCH_INDEX = [
  ...
  },
  {
    ...
    "body": "Data refreshes Your CRM data changes constantly — new donors, updated emails, fresh gift records. This page covers how to keep your Storyraise constituent list in step. Refreshing on demand To refresh a connection right now, open the Connections tab and click Sync now (the sync icon) on its card. Raiser's Edge NXT asks which constituent list to sync. If your authorization has expired, you may be asked to reconnect first. Salesforce asks who should sync: everyone, or one of your Contact List Views. Slate asks you to map your query's columns to first name, last name, and email. CSV — export a fresh file from your system and upload it again from the Constituents area. Because constituents are identified by email address, refreshed records update existing constituents rather than creating duplicates. New people in the source are added. When to refresh A good rule of thumb: refresh right before you do anything that depends on the data — before generating personalized links or QR codes for a mailing, before publishing a personalized report, before reviewing constituent engagement in Analytics for a board update. Automatic refreshes Storyraise can refresh a connection on a schedule, so you're not relying on someone remembering to press Sync now . Turn it on per connection: in the Connections tab, open a connection's menu and switch on Auto-sync . Pick Daily or Weekly , and the card then shows when it last ran. Each scheduled run does two things, in this order: Re-pulls your constituents from the source system, so a donor added to your CRM last week becomes a constituent without anyone opening the dashboard. Refreshes your synced collections , using the constituent list it just updated — so someone added tonight is linked to their rows in the same run rather than waiting for the next one. A scheduled run brings in the same people as your last Sync now : the Salesforce List View, the Raiser's Edge NXT list, or the Slate column mapping you chose. For Raiser's Edge NXT and Slate, run Sync now once after turning on Auto-sync, so Storyraise knows which list or mapping to use. Auto-sync is off until you turn it on, and it's set per connection, so you can leave a hand-curated source alone while a busy one refreshes nightly. Personalized links pick up new donors automatically. A report looks each reader up when they open their link, so a donor who syncs in tonight can open a report you published months ago. There's nothing to republish. Related Troubleshooting sync issues Importing constituent data Automated donor journeys — personalized reports sent by your email platform's automations, for donors who sync in overnight."
  },
  {
  ...
  },
  {
    ...
    "description": "Host your Storyraise portal at your own web address, such as portal.yourdomain.org, with a few DNS records your IT team adds once.",
    "section": "Account & Settings",
    "url": "/docs/account-and-settings/custom-portal-subdomain/",
    "keywords": [
      "setting",
      "custom",
      "subdomain",
    ...
    "body": "Setting up a custom subdomain for your portal Host your Storyraise portal at your own web address, such as portal.yourdomain.org, with a few DNS records your IT team adds once. A custom subdomain puts your organization's name in the address bar, so donors land on a site they recognize and trust. We handle the hosting side, and your IT team adds the DNS records we send. Most setups finish within a few hours, and your current portal address keeps working the whole time. Setting up email sending instead? That uses a different subdomain and a different set of records. See Setting up your email sending subdomain . Before you start You'll need: Someone who can edit DNS for your domain. Usually your IT team, or whoever manages your account with your domain provider (GoDaddy, Cloudflare, Network Solutions, and so on). The subdomain you want to use. See step 1. 1. Choose a subdomain Pick a subdomain that isn't in use today. The portal takes over the whole hostname, so anything served there now will stop being reachable. Common examples: portal.yourdomain.org donors.yourdomain.org giving.yourdomain.org Tip: Use a subdomain rather than your main domain ( yourdomain.org ) or www . Your main website stays exactly where it is. 2. We'll send you the DNS records Send us the subdomain you chose. We register it in our system, which generates the records for your domain, and we send them to you. Expect two: Type Host What it does TXT Your subdomain Confirms your organization owns the domain A Your subdomain Points the subdomain at our system The values are unique to your setup. Wait for ours rather than copying values from another article or another customer. 3. Add the records In your domain provider: Clear the hostname first. Delete any existing A, AAAA, or CNAME records on the subdomain. A leftover record sends some visitors to the old destination and blocks the security certificate. Add the records exactly as we sent them. Some providers want the full hostname ( portal.yourdomain.org ) in the Host field. Others add your domain automatically and want only the first part ( portal ). If you're not sure, check how your existing records are entered. Turn off proxying for the subdomain. If you use Cloudflare, set the record to DNS only (gray cloud). Other CDNs and web application firewalls need the same exception. Our system has to reach the subdomain directly to issue and renew its certificate. Check your CAA records. CAA records limit which certificate authorities can issue certificates for your domain. If your domain has them, add entries allowing letsencrypt.org and pki.goog . If it has none, skip this step. Good to know: Leave these records in place for as long as you use the subdomain. Our system checks them every time it renews the certificate, so removing them takes the portal offline. 4. Verification and certificate Once the records are added, let us know. Our system confirms them and then issues an SSL certificate for your subdomain automatically, so the portal loads securely over https:// . There's no certificate for you to buy, install, or renew. DNS changes usually take effect within a few hours. Allow up to 24 hours before troubleshooting. 5. Go live When the certificate is active, we switch your portal to the new address. Sign-in emails, invitations, and shared links all start using your subdomain. Your original portal address keeps working, so links you've already sent don't break. If people sign in with your organization's accounts If donors or staff sign in to the portal through your organization's identity provider (for example Auth0, Okta, or Microsoft Entra ID), that provider has to recognize the new address before sign-in works there. We'll send two URLs for whoever manages your identity provider to add to the same application, alongside the entries already there: A callback URL , where people return after signing in A logout URL , where people land after signing out Add them before we switch the portal over, so sign-in works the moment the new address goes live. Troubleshooting The subdomain shows the old site, or nothing at all. DNS changes can take a few hours to reach everyone. If it's been more than 24 hours, check that no old A, AAAA, or CNAME record is left on the subdomain. The browser shows a certificate warning or says the site isn't secure. The certificate is usually still being issued. If the warning lasts past 24 hours, check proxying (step 3.3) and CAA records (step 3.4). Those two cause nearly every certificate failure. Sign-in works at the old address but fails at the new one. Your identity provider is missing the new callback URL. See If people sign in with your organization's accounts above. Related Setting up your email sending subdomain Single sign-on (SSO) Network requirements and troubleshooting"
  },
  {
