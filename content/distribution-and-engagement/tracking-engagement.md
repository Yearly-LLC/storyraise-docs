...
- **Sections viewed** — which sections of the report each visitor reached.
- **Time spent** — how long visitors engage, overall and per section.
- **Link clicks** — taps on donation buttons, event links, social links, and any other link in your report, with what was clicked and when.
- **Location** — the visitor's region and city (never a precise address).
- **Device** — mobile or desktop, so you know how your audience actually reads.

All of this feeds the [Insights](understanding-report-metrics.md) page for each published report.
...
The upgrade comes from [personalized links](personalized-links.md): when a constituent opens their `?for=` link, the visit is attributed to them by name. That's what powers metrics like Constituents Reached and the Most Engaged Constituents list — and it's why personalized links are worth the small extra effort for any mailing where you care who engaged.
...
<!-- TEAM REVIEW: intent signals and time-spent metrics are part of the new Insights UI (insights_ui branch, pending merge). Confirm availability before publishing. -->
